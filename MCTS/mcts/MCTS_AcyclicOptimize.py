...
if __name__ == "__main__":
    ...
    model.eval()
    # Disable gradient computation for inference, which reduces memory usage and speeds up computation
    with torch.no_grad():
        difNumber1 = 0  # Counter for mismatches between prediction after full processing and ground truth
        difNumber0 = 0  # Counter for mismatches before any removal
        difNumber01 = 0  # Counter for mismatches after removing disputed votes but before MCTS refinement

        verified_input = matrices_original  # Use the original voting matrix before any deletion as the verification input
        verified_input = normalize_symmetric_elements(verified_input)  # Normalize the input to ensure symmetry
        verified_input = verified_input.to(device)  # Move data to the appropriate device (GPU or CPU)

        input1 = (currentState.votes_matrix).unsqueeze(0)  # Final matrix after MCTS, add batch dimension
        input1 = normalize_symmetric_elements(input1)
        input1 = input1.to(device)

        input0 = weightedVoteMatrix.unsqueeze(0)  # Raw weighted vote matrix before any disputed edge removal
        input0 = normalize_symmetric_elements(input0)
        input0 = input0.to(device)

        input01 = initVoteMatrix.unsqueeze(0)  # Matrix after removing disputed votes but before MCTS
        input01 = normalize_symmetric_elements(input01)
        input01 = input01.to(device)

        target = torch.tensor(rankings)  # Ground truth ranking
        target = target.to(device)

        # Pass inputs through the trained attention model
        verified_output, _ = model(verified_input)
        output1, _ = model(input1)
        output0, _ = model(input0)
        output01, _ = model(input01)

        # Convert model outputs into rankings
        # Since higher scores indicate lower ranks, use (1 - output) for proper sorting
        ...
        # Convert the output scores into ranking indices (e.g., [3,1,2] -> [1st,3rd,2nd])
        r_verified = rank_of_elements(v_verified)
        r1 = rank_of_elements(v1)
        r0 = rank_of_elements(v0)
        r01 = rank_of_elements(v01)

        ranking = (rankings[0] * M).tolist()  # Ground truth ranking in list form

        # Print and compare various ranking results
        print("Ground truth ranking:", ranking)
        print("Model output on original (ground truth) votes:", r_verified)
        print("Predicted ranking before removing disputed votes:", r0)
        print("Predicted ranking after removing disputed votes (before MCTS):", r01)
        print("Predicted ranking after removing disputed votes + MCTS:", r1)

        # Count how many positions differ between predicted rankings and the ground truth
        for j in range(M):
            if r0[j] != ranking[j]:
                difNumber0 += 1
        for j in range(M):
